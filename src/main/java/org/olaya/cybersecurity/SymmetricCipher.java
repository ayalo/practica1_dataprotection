...
public class SymmetricCipher {
    ...
    public byte[] encryptCBC (byte[] input, byte[] byteKey) throws Exception {
        ...
        System.out.println("Encript imprimo array 1d ciphertext :");
    ...
    public byte[] decryptCBC (byte[] input, byte[] byteKey) throws Exception {
        ...
        System.out.println("Decript ec.length:"+ ec.length+" Contenido ec");
        System.out.println(Arrays.deepToString(ec));
        System.out.println(" Contenido cipher2d :");
        System.out.println(Arrays.deepToString(cipher2d));

        for(int i=1;i<ec.length;i++){
            System.out.println( "Iteracion i="+i);
            System.out.println(Arrays.deepToString(cipher2d));
            System.out.println( "ec [i]"+ec[i].toString());
            System.out.println( "ec [i-1]"+ec[i-1].toString());
            cipher2d[i]=xor(d.decryptBlock(ec[i]),ec[i-1]);
            System.out.println( "Despues Iteracion i="+i);
        ...
        System.out.println(Arrays.deepToString(cipher2d));
        System.out.println("Decript imprimo array 1d ciphertext :");
        System.out.println(Arrays.toString(finalplaintext));
    ...
    */
    public byte[][] addPadding (byte[] input, boolean isEncript) throws Exception {
        System.out.println(" Estoy en AddPading , input length  "+input.length);
        int len_withpad=0;
        int len_pad=0;
        //int len_withpad= input.length + block_size -(input.length % block_size);// ej. 32
        if (isEncript) {
           len_withpad = input.length + block_size - (input.length % block_size);// ej. 32 char mete bloque de mas si se ajusta al tamaño
            len_pad= block_size -(input.length % block_size); //5 char  . Variable global para usarse en el decript unPadding
        } else {
            len_withpad = input.length;
            len_pad=0;
        }
        //int len_pad= block_size -(input.length % block_size); //5 char  . Variable global para usarse en el decript unPadding
        ...
        System.out.println(" add pading despues de inicialicial : "+Arrays.deepToString(ec));
        ...
        //    try{
              ...
              //  if (input.length % block_size != 0) {
                    ...
                    System.out.println("Adding padding imprimo array 2d ec de dimensiones " + ec.length + " :");
